...
We still learn the type of what we've entered (helpful for understanding where we've gone wrong), but since it doesn't match the goal type of `Number`, the editor won't let us select it! This experience of not being allowed to select a search result in the explorer is the closest we get to a type error.
