...
Now we can understand the `Map (h,h) (Set h)` used to represent the `opened` portion of the `Edit`. In our example, there would be an entry `((wrangle, wrangle'), {wrangleUser'})`. It is keyed by the `(old,new)` hash for the term that is open, and the `Set h` is any immediate dependents. Interacting with the edit just modifies this `opened` portion in predictable ways.
...
instance Ord h => Monoid (Edit h) where
  ...
  mappend (Edit c1 o1) (Edit c2 o2) = Edit c o
    ...
    o = Map.unionWith Set.union o1' o2'
    -- subtract from `o2` any entries closed by `c1`
    -- and subtract from `o1` any entries closed by `c2`
    o2' = Map.filterWithKey (\e _ -> keep c1 e) o2
    o1' = Map.filterWithKey (\e _ -> keep c2 e) o1
```

This is important. We have a _commutative merge function_ for our `Edit` type. The implementation merges corresponding keys in `closed`. And any `opened` values that are closed by the other side of the edit are closed. Any values open in both remain opened. Thus if someone else completes an edit you get their work, unless you've taken the root term in a different direction in which case it is preserved.
...
But, after thinking about it some more, I no longer think this sort of versioning is necessary, and I'm skeptical it's the right thing anyway. There are several reasons.

First, I no longer feel it's _necessary._ This is sort of a subtle point. In our current workflows, when code edits are shared asynchronously, with sometimes hours or days passing between merges, having more changes merge cleanly is a win. But, if we can share our changes more frequently, even in real-time, we don't care as much about clean merges. 'Conflicts' are detected much much earlier, and when they happen they are entirely a consequence of bad communication---people literally working on upgrading the same nodes in the dependency graph.
